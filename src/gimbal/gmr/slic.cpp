...
//==============================================================================
void SLIC::RGB2XYZ(const int &sR,
                   const int &sG,
                   const int &sB,
                   double &X,
                   double &Y,
...
///	RGB2LAB
//===========================================================================
void SLIC::RGB2LAB(const int &sR,
                   const int &sG,
                   const int &sB,
                   double &lval,
                   double &aval,
...
/// For whole volume
//===========================================================================
void SLIC::DoRGBtoLABConversion(unsigned int **&ubuff,
                                double **&lvec,
                                double **&avec,
                                double **&bvec) {
  int sz = m_width * m_height;
     ...
     height) )
                                  {
                                          ...
                                          if( false == istaken[index] )//comment
     this to obtain internal contours
                                          {
                                                  if( labels[mainindex] !=
     labels[index] ) np++;
                                          }
...
///	DetectLabEdges
//==============================================================================
void SLIC::DetectLabEdges(const double *lvec,
                          const double *avec,
                          const double *bvec,
                          const int &width,
                          const int &height,
...
///	PerturbSeeds
//===========================================================================
void SLIC::PerturbSeeds(vector<double> &kseedsl,
...
//===========================================================================
void SLIC::GetLABXYSeeds_ForGivenStepSize(vector<double> &kseedsl,
                                          vector<double> &kseedsa,
                                          vector<double> &kseedsb,
                                          vector<double> &kseedsx,
                                          vector<double> &kseedsy,
...
/// The k seed values are taken as uniform spatial pixel samples.
//===========================================================================
void SLIC::GetKValues_LABXYZ(vector<double> &kseedsl,
                             vector<double> &kseedsa,
                             vector<double> &kseedsb,
                             vector<double> &kseedsx,
                             vector<double> &kseedsy,
...
void SLIC::PerformSuperpixelSLIC(vector<double> &kseedsl,
  ...
  //----------------
  int offset = STEP;
  // if(STEP < 8) offset = STEP*1.5;//to prevent a crash due to a very small
  // step size
  //----------------
  ...
  for (int itr = 0; itr < 10; itr++) {
    ...
    for (int n = 0; n < numk; n++) {
      ...
      for (int y = y1; y < y2; y++) {
        for (int x = x1; x < x2; x++) {
          ...
          //------------------------------------------------------------------------
          dist += distxy * invwt; // dist = sqrt(dist) +
                                  // sqrt(distxy*invwt);//this is more exact
          //------------------------------------------------------------------------
          if (dist < distvec[i]) {
            distvec[i] = dist;
            klabels[i] = n;
...

    {
      for (int k = 0; k < numk; k++) {
        if (clustersize[k] <= 0)
          clustersize[k] = 1;
        inv[k] = 1.0 / clustersize[k]; // computing inverse now to multiply,
                                       // than divide later
      }
    }
...
///
///	Performs k mean segmentation. It is fast because it searches locally,
/// not
/// over the entire image.
//===========================================================================
void SLIC::PerformSupervoxelSLIC(vector<double> &kseedsl,
                                 vector<double> &kseedsa,
                                 vector<double> &kseedsb,
                                 vector<double> &kseedsx,
                                 vector<double> &kseedsy,
  ...
  double invwt =
      1.0 / ((STEP / compactness) *
             (STEP / compactness)); // compactness = 20.0 is usually good.
...
void SLIC::SaveSuperpixelLabels(const int *&labels,
  ...
  if (pos != string::npos) // if a slash is found, then take the filename with
                           // extension
  {
    nameandextension = filename.substr(pos + 1);
  }
  string newname = nameandextension.replace(nameandextension.rfind(".") + 1,
                                            3,
                                            "dat"); // find the position of the
                                                    // dot and replace the 3
      ...
      for (int h = 0; h < height; h++) {
        for (int w = 0; w < width; w++) {
          if (nlabels[d][i] < 0) {
            ...
            for (int c = 0; c < count; c++) {
              for (int n = 0; n < 10; n++) {
                ...
                if ((x >= 0 && x < width) && (y >= 0 && y < height) &&
                  ...
                  if (0 > nlabels[z][nindex] &&
                      labels[d][i] == labels[z][nindex]) {
                    xvec[count] = x;
                    yvec[count] = y;
                    zvec[count] = z;
            ...
            if (count <=
                (SUPSZ >>
                 2)) // this threshold can be changed according to needs
            {
              for (int c = 0; c < count; c++) {
                int ind = yvec[c] * width + xvec[c];
...
/// using bitwise operators as can be seen in the function
/// DoRGBtoLABConversion().
///
...
void SLIC::DoSuperpixelSegmentation_ForGivenSuperpixelSize(
  ...
  //--------------------------------------------------
  bool perturbseeds(true); // perturb seeds is not absolutely necessary, one can
                           // set this flag to false
  vector<double> edgemag(0);
  if (perturbseeds)
    DetectLabEdges(m_lvec, m_avec, m_bvec, m_width, m_height, edgemag);
  GetLABXYSeeds_ForGivenStepSize(kseedsl,
                                 kseedsa,
                                 kseedsb,
                                 kseedsx,
                                 kseedsy,
  ...
  EnforceLabelConnectivity(klabels,
                           m_width,
                           m_height,
                           nlabels,
                           numlabels,
    ...
    const double &compactness) // weight given to spatial distance
{
  ...
  DoSuperpixelSegmentation_ForGivenSuperpixelSize(ubuff,
                                                  width,
                                                  height,
                                                  klabels,
                                                  numlabels,
...
void SLIC::DoSupervoxelSegmentation(unsigned int **&ubuffvec,
  ...
  EnforceSupervoxelLabelConnectivity(klabels,
                                     width,
                                     height,
                                     depth,
                                     numlabels,
